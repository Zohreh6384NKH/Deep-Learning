...
#     system_message="Summarize the provided Python content and classify it into one of the following categories: "
#                    "data-related, configs, utils, training-related, or evaluation-related.",
...
# ###Resolve Import Paths for Absolute & Relative Imports**
...
# def summarize_content(class_or_function_contents: Dict[str, Dict[str, str]]) -> Dict[str, str]:
#     """Summarize the content of each class or function using the LLM-based summarization agent."""
#     summaries = {}
#     for file, definitions in class_or_function_contents.items():
#         for name, content in definitions.items():
#             try:
#                 prompt = (
#                     f"Summarize the following Python class or function and determine its category: "
#                     f"(data-related, configs, utils, training logic, evaluation logic):\n\n"
#                     f"{content[:2500]}"
#                 )
...
#                     summaries[f"{file}::{name}"] = response["content"]
#                 else:
#                     summaries[f"{file}::{name}"] = f"Unexpected response format: {response}"
#             except Exception as e:
#                 summaries[f"{file}::{name}"] = f"Error summarizing content: {str(e)}"
#     return summaries
...
# ### Process Imports and Extract Content**
# def process_imports(imports: Dict[Path, List[Dict[str, str]]]):
#     """Resolve, extract, and summarize both absolute and relative imports."""
#     class_or_function_contents = {}

#     for py_file, import_list in imports.items():
#         print(f"\n Scanning imports in: {py_file}")
...
#                 class_or_function_contents.setdefault(resolved_path, {})[import_item["name"]] = content
#             else:
#                 content = extract_class_or_function_content(resolved_path)
#                 class_or_function_contents[resolved_path] = {"__module__": content}
    

#     # Summarize extracted content**
#     summaries = summarize_content(class_or_function_contents)

#     return class_or_function_contents, summaries
...
#         imports[file_path] = extract_imports(file_path, REPO_ROOT)
#     print(f"imports: {imports}")

#     class_or_function_contents, summaries = process_imports(imports)
    


#     print("\n **Summarized Class/Function Contents:**")
...
def summarize_content(class_or_function_contents: Dict[str, str]) -> Dict[str, str]:
    ...
    for name, content in class_or_function_contents.items():
        ...
        try:
            ...
            print(f"⚠️ Error categorizing content: {str(e)}")
...
if __name__ == "__main__":
    ...
    print(f"📂 Scanning repository: {FOLDER_TO_SCAN.resolve()}")
